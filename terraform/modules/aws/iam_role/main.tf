...
resource "aws_iam_policy" "packer_build" {
  ...
  policy = jsonencode({
    ...
    Statement = [
      {
        // 破壊・変更系: Manager=packer タグ付きリソースに限定
        Sid    = "MutatePackerTaggedResources"
        Effect = "Allow"
        Action = [
          "ec2:AttachVolume",
        ...
        Condition = {
          StringEquals = {
            "aws:ResourceTag/Manager" = "packer"
            "aws:RequestedRegion"     = "ap-northeast-1"
          }
        }
      },
      {
        // タグ付き生成系: Packer が TagSpecifications で Manager=packer を付与する API
        Sid    = "CreateWithManagerTag"
        Effect = "Allow"
        Action = [
          "ec2:CopyImage",
          "ec2:CreateImage",
        ...
        Condition = {
          StringEquals = {
            "aws:RequestTag/Manager" = "packer"
            "aws:RequestedRegion"    = "ap-northeast-1"
          }
        }
      },
      {
        // 生成時にタグが付与されない API（temp SG / KeyPair など）
        Sid    = "CreateWithoutTagging"
        Effect = "Allow"
        Action = [
          "ec2:CreateKeyPair",
      ...
      {
        // Describe 系: resource-level 制限非対応
